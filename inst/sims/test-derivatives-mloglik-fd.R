...
#
# same as above but defining one of the parameters 
# in the slot 'cpar'
#
...
m <- stsm.model(model = "BSM", y = y, 
  pars = pars, nopars = nopars, cpar = cpar,
  transPars = transP)
...
g <- grad(func = mloglik.fd, x = m@pars, model = m)
g
h <- hessian(func = mloglik.fd, x = m@pars, model = m)
h
